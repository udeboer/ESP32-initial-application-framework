...
void systemtime_start() {
  ...
  send_queue = program_task_queue();
  // Create event
  static prog_task_queue_item_t signal_to_send;
  signal_to_send.prog_task_signal = minute_passed;
  // Set Timezone
  ...
  while (1) {
    ...
    tzset_patch();
    gettimeofday(&now, NULL);
    // current_timeinfo is used elsewhere. Seconds are not correct. Because not updated every
    // second.
    localtime_patch(&now.tv_sec, &current_timeinfo);
    ...
    if (last_sntp_sync_min() == 1) {
        ESP_LOGI(TAG, "SNTP updated the time");
#ifdef SNTP_MAX_TIME_NOSYNC
